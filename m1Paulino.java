...
BE SURE TO CHANGE THE NEXT SEVERAL LINES, USING YOUR REAL NAME.
    ...
    MY FIRST NAME IS......Dairo
    MY MIDDLE NAME IS.....Anthony
    MY LAST NAME IS.......Paulino
...
void bounce() {
  dogX += dogDX;  if ( dogX<west || dogX>east ) dogDX *= -1;
  dogY += dogDY;  if ( dogY<north || dogY>south ) dogDY *=  -1; 
  
  aceX += aceDX;  if ( aceX<west || aceX>east ) aceDX *= -1;
  aceY += aceDY;  if ( aceY<north || aceY>south ) aceDY *=  -1;
 
 pigX += pigDX;  if ( pigX<west || pigX>east ) pigDX *= -1;
 pigY += pigDY;  if ( pigY<north || pigY>south ) pigDY *=  -1;
...
void show() {
   fill( 250,30,0 );    ellipse( dogX+250,dogY+150, 30,30 ); 
   fill(0,150,0);       ellipse( aceX+250,aceY+200, 30,30);
   fill(0,0,100);       ellipse( pigX+250,pigY+250, 30,30);   //++++ ADD YOUR OWN CODE HERE.balls ellipse and fill ++++
}
void buttons() {
    //++++ ADD YOUR OWN CODE HERE. ++++
}
